...
export default function App() {
  ...
  useEffect(() => {
    ...
    const savedWhatsAppConfig = localStorage.getItem("whatsapp_signal_broadcaster_config");
    if (savedWhatsAppConfig) {
      try {
        const parsed = JSON.parse(savedWhatsAppConfig);
        setWhatsAppConfig({
          enableScannerBroadcast: true,
          enableManualBroadcast: true,
          ...parsed
        ...
        localStorage.removeItem("whatsapp_signal_broadcaster_config");
      }
    }
  ...
  const persistWhatsAppConfig = (newConfig: WhatsAppConfig) => {
    const normalizedConfig = {
      ...newConfig,
      apiTokenInstance: newConfig.apiTokenInstance.trim(),
      idInstance: newConfig.idInstance.trim(),
      chatId: newConfig.chatId.trim(),
    };
    setWhatsAppConfig(normalizedConfig);
    localStorage.setItem("whatsapp_signal_broadcaster_config", JSON.stringify(normalizedConfig));
  };
  ...
  const executeAutoBroadcast = async (draftData: {
    ...
    try {
      ...
      if (whatsAppConfig.isConnected && whatsAppConfig.enableManualBroadcast !== false) {
        try {
          ...
          if (waRes.ok && waData.success) {
            waMessageId = waData.idMessage ? String(waData.idMessage) : null;
          }
        } catch (waErr) {
          console.error("Auto broadcast to WhatsApp failed:", waErr);
        }
      }
      ...
      const newSignal: TradingSignal = {
        ...
        whatsappChatIdUsed: whatsAppConfig.chatId,
    ...
    try {
      ...
      if (whatsAppConfig.isConnected && whatsAppConfig.enableManualBroadcast !== false) {
        try {
          ...
          console.error("Manual broadcast to WhatsApp failed:", waErr);
        }
      }
  ...
  const handlePostDirectTelegram = async (text: string, bypassScannerCheck = false) => {
    if (!bypassScannerCheck && config.enableScannerBroadcast === false) {
    ...
    try {
      ...
      let waMessageId: string | null = null;
      if (whatsAppConfig.isConnected && (bypassScannerCheck || whatsAppConfig.enableScannerBroadcast !== false)) {
        try {
          const waRes = await fetch("/api/whatsapp/send", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          ...
          console.error("Scanner broadcast to WhatsApp failed:", waErr);
        }
      }
  ...
  const handleSendSignalUpdate = async (updateType: "TP1" | "TP2" | "TP3" | "SL" | "CLOSED" | "CUSTOM") => {
    ...
    try {
      ...
      // Deliver update to WhatsApp if configured
      if (whatsAppConfig.isConnected) {
        try {
          ...
          console.error("Signal update to WhatsApp failed:", waErr);
        }
      }
            ...
            {activeTab === "dashboard" ? (
              ...
              >
                <SettingsView
                  ...
                  onWhatsAppChange={persistWhatsAppConfig}
                  aiConfigured={aiConfigured}
                />
              </motion.div>
            ) : (
