...
interface Props {
  ...
  onPostDirectTelegram?: (text: string, bypassScannerCheck?: boolean) => Promise<{ success: boolean; messageId?: string; error?: string }>;
...
export default function TradingSignalForm({ 
  ...
  // --- AI RAW DATA PARSER STATES & FUNCTION ---
  const [rawTextPrompt, setRawTextPrompt] = useState("");
  const [isParsingPhases, setIsParsingPhases] = useState(false);
  const [parseError, setParseError] = useState("");
  const [parseSuccess, setParseSuccess] = useState("");

  const handleParseRawPhases = async () => {
    if (!rawTextPrompt.trim()) {
      setParseError("Please provide some raw trading text data first!");
      return;
    }

    setIsParsingPhases(true);
    setParseError("");
    setParseSuccess("");

    try {
      const response = await fetch("/api/gemini/parse-phases", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ rawText: rawTextPrompt }),
      });

      const data = await response.json();
      if (!response.ok || data.error) {
        throw new Error(data.error || "Failed to parse raw data.");
      }

      // Update basic sequence parameters
      if (data.symbol) {
        setSeqSymbol(data.symbol);
        setHqSignalName(data.symbol + " Sequence Trigger");
      }
      if (data.action) {
        setSeqAction(data.action);
        setHqCurrentTrigger(`Active Setup Triggered: ${data.action}`);
      }
      if (data.strategy) {
        setSeqStrategy(data.strategy);
        setHqLeadTrigger(`Scan condition detected via ${data.strategy}.`);
      }

      // Update HTML editable templates
      if (data.preSignal) {
        setSeqPreSignal(data.preSignal);
      }
      if (data.activeSignal) {
        setSeqActiveSignal(data.activeSignal);
      }
      if (data.postSignal) {
        setSeqPostSignal(data.postSignal);
      }

      setParseSuccess("Successfully analyzed and generated 3-phase alert templates below!");
      setRawTextPrompt(""); // clear input on success
    } catch (err: any) {
      console.error(err);
      setParseError(err.message || "Something went wrong while formulating sequence.");
    } finally {
      setIsParsingPhases(false);
    }
  };
  ...
  const handleTransmitDirect = async (text: string, type: "pre" | "active" | "post") => {
    ...
    try {
      const res = await onPostDirectTelegram(text, true);
      if (res.success) {
        if (type === "pre") {
      ...
      <form onSubmit={handleGeminiFormat} className="space-y-4">
        {formMode === "deriv" ? (
          ...
          <div className="space-y-6 animate-fade-in text-sans" id="sequence-form-fields">
            ...
            {/* ✨ AI RAW DATA & PROMPT PHASE GENERATOR CARD ✨ */}
            <div className="bg-slate-900/90 p-5 border border-slate-800 rounded-xl space-y-4">
              ...
              <div className="space-y-3 font-sans">
                ...
                <div className="space-y-1.5">
                  <textarea
                    ...
                    placeholder="e.g., V100 Buy standby now. Set trigger at 2541.5, target 2580, stop 2520. Cooldown scheduled for 2 hours after fill."
                    className="w-full p-3 text-xs bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-lg text-slate-100 placeholder-slate-650 outline-none leading-relaxed font-mono"
                    disabled={isParsingPhases}
                  />
                </div>

                {parseError && (
                  <div className="bg-rose-950/30 text-rose-300 border border-rose-900/50 rounded-xl p-3 text-xs flex items-center gap-2">
                ...
                {parseSuccess && (
                  <div className="bg-emerald-950/30 text-emerald-300 border border-emerald-900/50 rounded-xl p-3 text-xs flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                ...
                <div className="flex items-center justify-end">
                  <button
                    type="button"
                    onClick={handleParseRawPhases}
                    disabled={isParsingPhases || !rawTextPrompt.trim()}
                    className="px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 disabled:from-slate-800 disabled:to-slate-800 disabled:text-slate-500 text-white text-xs font-semibold rounded-lg shadow transition-all cursor-pointer flex items-center gap-1.5"
                  >
                    {isParsingPhases ? (
                      <>
                        ...
                        <span>Formulating 3-Phases...</span>
                      </>
                    ) : (
                      <>
                        <Sparkles className="w-3.5 h-3.5 text-white fill-white/10" />
                        <span>Formulate 3-Phase Sequences ✨</span>
                      </>
                    )}
                  </button>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div className="bg-slate-950/80 border border-emerald-900/30 rounded-xl p-4.5 space-y-4 hover:border-emerald-900/60 transition-all flex flex-col justify-between">
                <div className="space-y-3.5">
                  ...
                  <div className="space-y-3 font-sans">
                    ...
                    <div className="space-y-1 pt-2 border-t border-slate-900/60">
                      <label className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider flex items-center justify-between">
                        <span>Phase 1 Final HTML Message</span>
                        <span className="text-[9px] text-slate-500 normal-case font-normal">Editable</span>
                      </label>
                      <textarea
                        rows={5}
                        value={seqPreSignal}
                        onChange={(e) => setSeqPreSignal(e.target.value)}
                        placeholder="Phase 1 HTML content..."
                        className="w-full p-2 text-[11px] bg-slate-950 border border-slate-900 focus:border-emerald-500 rounded-lg text-emerald-350 font-mono resize-y leading-relaxed outline-none"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-slate-900/85 mt-2 flex flex-col gap-1.5">
                  <button
                    type="button"
                    onClick={() => handleTransmitDirect(seqPreSignal, "pre")}
              ...
              <div className="bg-slate-950/80 border border-sky-900/30 rounded-xl p-4.5 space-y-4 hover:border-sky-900/60 transition-all flex flex-col justify-between">
                  ...
                  <div className="space-y-3 font-sans">
                    ...
                    <div className="space-y-1 pt-2 border-t border-slate-900/60">
                      <label className="text-[10px] font-bold text-sky-400 uppercase tracking-wider flex items-center justify-between">
                        <span>Phase 2 Final HTML Message</span>
                        <span className="text-[9px] text-slate-500 normal-case font-normal">Editable</span>
                      </label>
                      <textarea
                        rows={5}
                        value={seqActiveSignal}
                        onChange={(e) => setSeqActiveSignal(e.target.value)}
                        placeholder="Phase 2 HTML content..."
                        className="w-full p-2 text-[11px] bg-slate-950 border border-slate-900 focus:border-sky-500 rounded-lg text-sky-350 font-mono resize-y leading-relaxed outline-none"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-slate-900/85 mt-2 flex flex-col gap-1.5">
                  <button
                    type="button"
                    onClick={() => handleTransmitDirect(seqActiveSignal, "active")}
              ...
              <div className="bg-slate-950/80 border border-amber-900/30 rounded-xl p-4.5 space-y-4 hover:border-amber-900/60 transition-all flex flex-col justify-between">
                  ...
                  <div className="space-y-3 font-sans">
                    ...
                    <div className="space-y-1 pt-2 border-t border-slate-900/60">
                      <label className="text-[10px] font-bold text-amber-400 uppercase tracking-wider flex items-center justify-between">
                        <span>Phase 3 Final HTML Message</span>
                        <span className="text-[9px] text-slate-500 normal-case font-normal">Editable</span>
                      </label>
                      <textarea
                        rows={5}
                        value={seqPostSignal}
                        onChange={(e) => setSeqPostSignal(e.target.value)}
                        placeholder="Phase 3 HTML content..."
                        className="w-full p-2 text-[11px] bg-slate-950 border border-slate-900 focus:border-amber-500 rounded-lg text-amber-350 font-mono resize-y leading-relaxed outline-none"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-slate-900/85 mt-2 flex flex-col gap-1.5">
                  <button
                    type="button"
                    onClick={() => handleTransmitDirect(seqPostSignal, "post")}
