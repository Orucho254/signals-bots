...
import { TelegramConfig, WhatsAppConfig } from "../types";

interface Props {
  config: TelegramConfig;
  onChange: (cfg: TelegramConfig) => void;
  whatsAppConfig: WhatsAppConfig;
  onWhatsAppChange: (cfg: WhatsAppConfig) => void;
...
export default function SettingsView({ config, onChange, whatsAppConfig, onWhatsAppChange, aiConfigured }: Props) {
    ...
    try {
      ...
      const res = await fetch("/api/autobroadcast/configure", {
        ...
        body: JSON.stringify({
          ...
          whatsappChatId: whatsAppConfig.isConnected ? whatsAppConfig.chatId : undefined,
        }),
      });
  ...
  return (
    <div className="space-y-6 animate-fade-in" id="settings-view-panel">
      ...
      {/* WhatsApp connection */}
      <div className="bg-slate-950 p-2.5 rounded-2xl border border-slate-900 shadow-xl overflow-hidden">
        <WhatsAppConfigPanel config={whatsAppConfig} onChange={onWhatsAppChange} />
      </div>
